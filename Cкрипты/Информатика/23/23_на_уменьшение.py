...
def solve_task(
    ...
    if x == out:
        ...
        if cnt_of_com == 2:
            return solve_task(eval(str(x) + coms[0]), y, out, coms, cnt_of_com) + solve_task(eval(str(x) + coms[1]), y,
                                                                                             out, coms, cnt_of_com)
        elif cnt_of_com == 3:
            return solve_task(eval(str(x) + coms[0]), y, out, coms, cnt_of_com) + solve_task(eval(str(x) + coms[1]), y,
                                                                                             out, coms,
                                                                                             cnt_of_com) + solve_task(
                eval(str(x) + coms[2]), y, out, coms, cnt_of_com)
...
while True:
    ...
    # -- возможные команды
    possible_commands = ['Вычесть', 'Разделить с округлением вниз на']
    # -- количество команд
    ...
    numbers_for_division = [int(x) for x in range(2, 4)]
    ...
    for i in range(len(commands)):
        if commands[i] == possible_commands[0]:
            ...
            n = random.choice(numbers_for_division)
            numbers.append(n)
            commands[i] = commands[i] + f' {n}'
    ...
    for s in commands:
        if 'Вычесть' in s:
            ...
            s = s.replace('Разделить с округлением вниз на', '//')
        coms.append(s)
...
"""
    ...
    right_answer = solve_task(final_num, in_trajectory, out_trajectory, coms, cnt_of_command) * solve_task(
        in_trajectory, start_num, out_trajectory, coms, cnt_of_command)
...
print(solve_task({final_num}, {in_trajectory}) * solve_task({in_trajectory}, {start_num}))
"""
